...
import { supabase, type YoutubeVideo } from "@/lib/supabase";
...
const FeaturedVideos = () => {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const { data, error } = await supabase.functions.invoke<{ videos: YoutubeVideo[] }>(
        "get-youtube-videos",
      );
      if (cancelled) return;
  ...
  return (
    <section id="latest" className="py-16 sm:py-24 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-bold mb-2">Latest from YouTube</h2>
